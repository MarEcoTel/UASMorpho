...
# will be renamed using the format yyyymmddhhmmss-DroneName.MOV. A new file titled LidarLog_DailyMaster.csv
...
# to the original files in addition to these newly made and named files. If you no longer need to reference the 
# folder structure used to quickly download and save data while in the field, you can delete those
# drone-named folders to save room on you computer or storage device. Be aware that the newly named videos are 
# not named with the exact start time of the video as marked by the mobile demands. This is because the DJI cameras' timing system
# is different from the GPS times by several seconds, and the number of seconds off changes with each video/image. 
# So, if you are going to be comparing lidar data times with videos using the file name timestamp, you will need to adjust
# for this timing issue. The corrected times for each video are in the tFlights spreadsheet within each daily 
# database. If you are having trouble getting the correct video start time, please let me, David, know and I'd be happy to help.
...
# There are likely ways to improve things, and any ideas are always welcome.
...
# David ideas for changes:
  # - allow manual input of drone folder names to allow subset of 
...
#### stuff that just needs to run ####
{
  ...
  for (f in c(METRInsp1_folders, METRInsp2_folders, METRMav1_folders)) { #iterate through dumped folders
    ...
    for (l in lfiles) { #iterate through lidar files
      ...
      if (nrow(lidar_log) > 0) {
        #organize lidar data
        lidar_log <- suppressWarnings(lidar_log %>% 
          mutate(gmt_time = as.character(gmt_time),
                 ...
                 Drone = drone) %>% 
          filter(gmt_time != "INVALID") %>% 
          rename(gmt_date = `#gmt_date`) %>% 
          mutate(DateTime = NA,
                 DateTime = if_else(gmt_time != "INVALID" & gmt_date != "INVALID", 
                                    ymd_hms(paste0("20", gmt_date, " ", gmt_time)),
                                    as.POSIXct(NA))))
        #add datetimes before GPS kicked into play
        if (length(which(is.na(lidar_log$DateTime))) > 0) {
          for (ndt in max(which(is.na(lidar_log$DateTime))):1) {
            lidar_log$DateTime[ndt] <- lidar_log$DateTime[ndt+1] - 1
          }
        }
        #combine all lidar data files
        lidar_master <- bind_rows(lidar_master, lidar_log)
      }
    }
  ...
  lidar_master <- lidar_master %>% mutate(DateTime = as.character(DateTime)) #convert dates to character strings
