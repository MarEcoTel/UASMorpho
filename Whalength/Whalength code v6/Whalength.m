...
%rename existing lengths output file if present
ds=handles.pathd;
split_ds=split(ds,'\');
ds_date=split_ds{end};
dir_ds=dir(ds);
if contains([dir_ds.name], strcat(ds_date, ' lengths.csv'))
    oldfile = strcat(ds, '\', strcat(ds_date, ' lengths.csv'));
    newfile = strcat(ds, '\', strcat(ds_date, ' lengths oldsince_', char(join(string(fix(clock)), '')), '.csv'));
    movefile(oldfile, newfile)
elseif contains([dir_ds.name], strcat(ds_date, ' lengths.xlsx'))
    oldfile = strcat(ds, '\', strcat(ds_date, ' lengths.xlsx'));
    newfile = strcat(ds, '\', strcat(ds_date, ' lengths oldsince_', char(join(string(fix(clock)), '')), '.xlsx'));
    movefile(oldfile, newfile)
elseif contains([dir_ds.name], strcat(ds_date, ' lengths.txt'))
    oldfile = strcat(ds, '\', strcat(ds_date, ' lengths.txt'));
    newfile = strcat(ds, '\', strcat(ds_date, ' lengths oldsince_', char(join(string(fix(clock)), '')), '.txt'));
    movefile(oldfile, newfile)
end
