...
#### Manual Inputs ####
day_folder <- "E:/IG/Sep-Oct 2021/UAS/2021-09-28"  #folder directory for days images
images_folder <- "E:/IG/Sep-Oct 2021/UAS/2021-09-28/Stills/Measured"  #folder directory where stills were stored
...
lidar_camera_offset_cm <- -9 #cm between lidar and camera lenses
...
#### stuff that just needs to run ####
{
  ...
  if (image_files[1] == "[Originals]") {
    image_files <- image_files[2:length(image_files)]
  }
  ...
  for (f in 1:length(video_files)) { #iterate through all necessary video files
    ...
    vid_stills <- vid_stills[grepl(".png", vid_stills)]
    vid_stills <- vid_stills[which(grepl("measurements", vid_stills) == FALSE)]
  ...
  #calculate corrected altitude with tilt
  excel_file <- excel_file %>% mutate(CorrectedAltitude = round((Lidar*cos(Tilt*pi/180)+lidar_camera_offset_cm)/100, 2))
